...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    ...
    <div class="btn-toolbar mb-2 mb-md-0">
        <a href="{{route('admin.user.regist')}}" class="btn btn-sm btn-outline-primary">新規作成</a>
    </div>
</div>
<div class="card">
...
@foreach( $rows as $row )
                <tr>
                    ...
                    <td>{{$row->name}}</td>
                    <td>{{$row->email}}</td>
                    <td>{{$def['type'][$row->type] ?? '管理者'}} </td>
                    <td>{{$row->updated_at}}<br/>{{$row->created_at}}</td>
                    <td>
                        <a href="{{route('admin.user.detail', $row->id)}}" class="btn btn-outline-primary">詳細</a>
                        <a href="{{route('admin.user.update', $row->id)}}" class="btn btn-outline-primary">編集</a>
                        <a href="{{route('admin.user.delete.confirm', $row->id)}}" class="btn btn-outline-primary">削除</a>
                    </td>
                </tr>
@endforeach
            </thead>
